...
// Define UserRole type or enum if needed
type UserRole = 'ADMIN' | 'USER'; // Adjust values to match your schema
...
class LedgerService {
   ...
   */
  async mint(userId: string, amountToMint: number, notes: string, externalTxnId: string) {
   ...
   */
  async requestWithdrawal(userId: string, amountToWithdraw: number) {
    const userWallet = await prisma.wallet.findUnique({
      where: { userId },
    });
   ...
   */
  async executePayout(internalTxnId: string, amount: number, destinationAddress: string) {
    ...
    try {
      ...
      await this.failTransaction(internalTxnId, (error as Error).message);
    }
  }
   ...
   */
  async completeTransaction(internalTxnId: string, externalTxnId: string) {
   ...
   */
  async failTransaction(internalTxnId: string, failureReason: string) {
